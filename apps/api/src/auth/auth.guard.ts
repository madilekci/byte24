...
import { ApplicationRoles } from './decorators';
...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const applicationRoles = this.reflector.get(ApplicationRoles, context.getHandler());

    //@ts-expect-error - session.user type is not extended with applicationRole
    if (applicationRoles && !applicationRoles.includes(session.user?.applicationRole)) {
